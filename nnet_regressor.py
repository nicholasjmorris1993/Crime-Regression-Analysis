...
class Regression:
    ...
    def fit(self, X, y):
        ...
        self.scaler1 = ScaleFeatures(self.scale, bounds=(0.2, 0.8))
        ...
        preprocessX = self.scaler1.fit_transform(preprocessX)
        ...
        preprocessX = self.scaler2.fit_transform(preprocessX)
        ...
        trainX = self.scaler1.transform(trainX)
        ...
        trainX = self.scaler2.transform(trainX)
        print("> Training Neural Network")
        if self.deep:
            hidden = [128, 128, 128, 128, 128, 128, 128, 128, 128, 128]
        else:
            hidden = [128, 128]
    ...
    def refit(self, X, y):
        ...
        print("> Training Neural Network")
        self.nnet.fit(self.X, self.y, epochs=500, batch_size=16, workers=-1, verbose=0)
    ...
    def importance(self, X, y):
        ...
        for col in X.columns:
            ...
            for i in range(iterations):
                X_perturb[col] = X[col] + mean * perturb[:,i]  # perturb
    ...
    def dump(self):
        ...
        with open(f"{self.name}/dump/scaler1", "wb") as f:
        ...
        with open(f"{self.name}/dump/scaler2", "wb") as f:
            pickle.dump(self.scaler2, f)
        self.nnet.save(f"{self.name}/dump/nnet.keras")
    ...
    def load(self):
        ...
        with open(f"{self.name}/dump/scaler1", "rb") as f:
        ...
        with open(f"{self.name}/dump/scaler2", "rb") as f:
            self.scaler2 = pickle.load(f)
        self.nnet = keras.models.load_model(f"{self.name}/dump/nnet.keras")
